...
const getAllStaffFromDB = async (userId: string) => {
  ...
  return await Staff.aggregate([
    ...
    { $match: { addedBy: new mongoose.Types.ObjectId(userId), isDeleted: false } },
...
const updateStaffIntoDB = async (id: string, payload: Partial<IStaff>, userId: string) => {
  const result = await Staff.findOneAndUpdate({ _id: id, addedBy: userId }, payload, { new: true });
  return result;
};

const deleteStaffFromDB = async (id: string, userId: string) => {
  const result = await Staff.findOneAndUpdate({ _id: id, addedBy: userId }, { isDeleted: true }, { new: true });
  return result;
};
